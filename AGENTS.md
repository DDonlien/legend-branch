...
- 项目名称：LegendBranch · 传说谱系
- 产品简介：用可交互时间图谱呈现传说角色、民俗母题、文学版本与现代作品形象如何传播、分支、合流和演化。
- 主要用户：对神话民俗、历史、文学、动漫游戏、叙事设计和世界观创作感兴趣的探索者。
- 当前阶段：P1 最小化 Demo 已完成；以“孙悟空 Trace”验证核心阅读体验和视觉语言。

### 技术栈与命令

- 技术栈：TypeScript + React + Vite；P1 图谱画布使用原生 SVG。
- 开发命令：`npm run dev`
- 测试命令：`npm run lint`
- 构建命令：`npm run build`
- 发布命令：尚未定义。
...
- 根目录：项目级协作和产品文档。
- 子功能目录：应用代码位于 `src/`，UI 组件位于 `src/components/`。
- 资源目录：P1 视觉参考图位于 `output/concepts/`。
- 文档目录：根目录 `README.md`、`REQUIREMENTS.md`、`DESIGN.md`；讨论记录和执行日志位于 `agent-log/`。
- 测试目录：尚未创建；当前通过 ESLint、生产构建和浏览器人工验收验证。
...
- 语言与命名：产品英文名为 `LegendBranch`，中文名为“传说谱系”，标语为 `Trace the evolution of legends.`；代码标识符使用英文，产品文档默认使用中文。
- 设计原则：优先保证高密度图谱的清晰、沉静和长时间可读性；主图视觉克制，Trace 状态才使用更醒目的高亮。
- 架构限制：图谱数据必须能表达多来源、多分支和合流，不把传播关系简化为树；横轴表示历史时间，纵轴由文化、国家或政权区块及角色行构成。
- 授权与引用边界：节点详情以简介和外部索引链接为主；外部资料、图片和文本必须遵守来源授权，不直接复制受版权保护的长文本。
- 安全、隐私或合规要求：AI 可辅助发现候选关系，但不得把 AI 推断直接当作事实发布；传播关系必须保留可信度和来源信息，并允许人工复核。
